...
if [[ "$MODE" == "FORCE" ]]; then
	...
	REPLAY_LOCATION_DIFF=$(sudo -iu postgres psql -p ${CLUSTER_PORT} postgres -Atc "SELECT pg_wal_lsn_diff(pg_last_wal_replay_lsn(), replay_lsn) from pg_stat_replication where application_name='s2s-replication'")
