...
# aws1-test1-* containers generated by salt
declare -a aws1test1containers=(
{%- for name in hosts|sort %}
{%- set host = hosts[name] %}
{%- if 'hidden' not in host or not host['hidden'] == True %}
{%- if 'absent' not in host or not  host['absent'] == True %}
...
if [[ " ${aws1test1containers[*]} " =~ " aws1-test1-${APP_TARGET} " ]]; then
    declare -A TARGET_WAR_APP_MAP
    TARGET_WAR_APP_MAP["aws1-salt1"]="aws1-test1-${APP_TARGET}"
    TARGET_WEB="aws1-test1-${APP_TARGET}"
fi
